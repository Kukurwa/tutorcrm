...
import { publishRequestSchema } from '@tutorcrm/contracts';
...
export async function POST(req: Request, { params }: Ctx) {
  ...
  const parsed = await parseJson(req, publishRequestSchema);
  ...
  try {
    ...
    if (current.stage === 'closed_won' || current.stage === 'closed_lost') {
      return errorResponse('CONFLICT', 'Cannot publish closed request');
    }
    const next = {
      ...current,
      publishedChannels: parsed.data.channels,
      publishedAt: nowIso(),
      stage: current.stage === 'request_created' ? ('published' as const) : current.stage,
      updatedAt: nowIso(),
    };
